...
let StarterLanguages = ["italian", "english"];
...
CeppiLinguistici["anglosassone"] = ["english", "german"];
...
CeppiLinguistici["greca"] = ["greek"];
CeppiLinguistici["ebraica"] = ["hebrew", "yiddish"];
CeppiLinguistici["ugrofinnico"] = ["Hungarian"];
...
var additionalOptions = {
  ...
  font: "https://eurotales.eu/wp-content/themes/emaurri/timeline/TimeLine_eng/MyCustomFont.css",
  // debug: true,
  //  hash_bookmark: false,
  // is_embed:false,
  // timenav_height:1000,
...
function GetFilteredFromMilestones(filter = null) {
  ...
  Milestones.forEach((m) => {
    if (Languages.includes(m.lingua.toLowerCase()) && m.Anno !== undefined) {
      ...
      var event_text = {};
      event_text.headline = m.name;
      event_text.text = m.name;
      start_date = {};
      var safeDate = m.Anno;
...
function GetInfoPopUp(Milestone) {
  ...
  if (Milestone.validita !== null) {
    ...
    try {
  document.getElementById("Subtitle").innerHTML = Milestone.name.toUpperCase();
...
function ReturnIconCategoria(cat) {
  if (cat.toLowerCase().includes("attestations")) {
    return "primeattestazioni.png";
  }
  if (cat.toLowerCase().includes("canonical")) {
    return "testicanonici.png";
  }
  if (cat.toLowerCase().includes("institutions")) {
    return "istituzioni.png";
  }
  if (cat.toLowerCase().includes("political")) {
    return "politici.png";
  }
  if (cat.toLowerCase().includes("media")) {
    return "media.png";
  }
  if (cat.toLowerCase().includes("gender")) {
    return "genere.png";
  } else {
    return null;
  }
