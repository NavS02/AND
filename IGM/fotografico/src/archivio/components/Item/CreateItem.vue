...
watch(
  ...
  () => {
    ...
    if (collection.value == "Foto") {
      // checkdefault();
      defaultWebValue();
      checkPositivoNegativo();
    }
  },
  { immediate: true, deep: true }
);

async function defaultWebValue() {
  const webToggle = fields.value.find((field) => field.name === "web");
  webToggle.value = true;
}
function checkPositivoNegativo() {
