<template v-if="field.invisible !== 'true'">
  ...
  <template v-else>
    <div
      class="buttons d-flex gap-2 border border-muted rounded p-2"
      v-if="field.edit !== 'false' && field.invisible !== 'true'"
    >
      <button
...
const useQuery = (query) => {
  ...
  if (related == "directus_roles") {
    ...
    params.filter.description = {
      _eq: "fotografico", // only show fotografico roles
    };
    return params;
  } else {
    const params = {};
