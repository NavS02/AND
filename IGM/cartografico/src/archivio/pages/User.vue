              ...
              <ul class="nav nav-tabs nav-tabs-bordered" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                  <button
                    class="nav-link"
                    data-bs-toggle="tab"
                    data-bs-target="#profile-favorites-items"
                    aria-selected="false"
                    role="tab"
                    tabindex="-1"
                    @click="fetchData()"
                ...
                <li class="nav-item" role="presentation">
                  ...
                  v-if="userRolePermissions.cartografico.delete"
                ...
                <li class="nav-item" role="presentation">
                  <button
                    class="nav-link"
                    data-bs-toggle="tab"
                    data-bs-target="#profile-deleted2-items"
                    aria-selected="false"
                    role="tab"
                    tabindex="-1"
                    @click="fetchDeletedItems2()"
                  v-if="userRolePermissions.cartografico.delete"
                ...
                >
                  ...
                  v-if="userRolePermissions.cartografico.edit"
                          ...
                          <button
                            title="Info"
                            class="btn btn-sm btn-light"
                            @click="onInfoClicked(item, 'carte')"
                          >
                ...
                >
                  ...
                  v-if="userRolePermissions.cartografico.edit"
                          ...
                          <!-- <button
                            title="Info"
                            class="btn btn-sm btn-light"
                            @click="onInfoClicked(item, 'serie')"
                          >
                            <font-awesome-icon icon="fa-solid fa-eye" />
                          </button> -->
                        </div>
...
export default {
  ...
  setup(props, context) {
    ...
    const counter = ref();
    const fieldsSerie = ref();
    const fieldsSerieF = ref();
    const deletedItems2 = ref();
    ...
    watch(
      route,
      () => {
        collection.value = "pref_c";
        collection4.value = "pref_s";
        ...
        const itemSettings2 = settings["carte_storiche"];
        const itemSettingsSerieF = settings["serie_italiana"];
        const itemSettingsSerie = settings[collection4.value];
        // // define the subset of fields you need to view in the table
        const collectionFields2 = itemSettings2.tableFields();
        const collectionFieldsSerie = itemSettingsSerie.tableFields();
        const collectionFieldsSerieF = itemSettingsSerieF.tableFields();
        console.log(collectionFieldsSerieF);
        fieldsSerie.value = collectionFieldsSerie;
        fieldsOpera.value = collectionFields2;
        fieldsSerieF.value = collectionFieldsSerieF;
        fetchData();
      },
    ...
    async function fetchData() {
      ...
      try {
        const response = await directus.items(collection4.value).readByQuery({
          filter: {
            user_created: {
              _eq: me.value.id,
            },
    ...
    async function fetchDataUsers() {
      if (
        me.value.role == "Admin Cartografico" ||
        ...
        const userList = await directus.items("directus_users").readByQuery({
          filter: { role: { description: { _eq: "cartografico" } } },
        });
        console.log(userList);
        itemsUsers.value = userList.data;
        const roleArray = await directus
          .items("directus_roles")
          .readByQuery({ filter: { description: { _eq: "cartografico" } } });
    ...
    async function onSaveClicked2(item) {
      await directus.items(collection4.value).deleteOne(item.id);
      fetchData();
    }
    function onEditClicked(item) {
      router.push({
        name: "editItemArc",
        params: { id: item.id_carta, collection: "carte_storiche" },
      });
    }
    ...
    function onEditClicked4(item) {
      router.push({
        name: "editItemArc",
        params: { id: item.id_serie, collection: "serie_italiana" },
      });
    }
    ...
    function onInfoClicked(item, type) {
      currentItem.value = item.id_carta;
      if (type == "carte") {
        router.push({
    ...
    async function fetchDeletedItems2() {
    ...
    async function recoverOpera(item) {
      await directus
        ...
        .updateOne(item.id, { status: "attivo" });
      fetchDeletedItems();
    }
    async function recoverOpera2(item) {
      await directus
    ...
    async function deleteOpera(item) {
      ...
      await store.collections.deleteOne("carte_storiche", id);
      fetchDeletedItems();
    }
    async function deleteOpera2(item) {
      const { id } = item;

      await store.collections.deleteOne("serie_italiana", id);
      fetchDeletedItems2();
    }

    function printSchedaC() {
      const { id } = item;
