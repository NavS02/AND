<template>
  <main id="main" class="main" v-if="userRolePermissions">
    ...
    <div class="col-12">
      ...
      <div class="row">
        <div class="col-md-6">
          <label class="form-label" :for="`form-foglio`">Foglio:</label>
          <input
            type="text"
            class="form-control"
            v-model="form.foglio"
      ...
      <div class="row">
        <div class="col-md-6">
          <label class="form-label" :for="`form-scala`">Scala:</label>
          <select
            class="form-select"
            v-model="form.scala"
            :id="`form-scala`"
        ...
        <div class="col-md-6">
          <label class="form-label" :for="`form-anno`">Anno:</label>
          <input
            type="text"
            class="form-control"
            v-model="form.anno"
                    ...
                    <div class="card-body">
                      ...
                      >
                        <img
                          :src="
                            imageurlBase +
                            '/cartografia/public/images/foto_digitales/' +
                            item.link
                          "
                          alt=""
...
const storageManager = useStorage("advanced-search-form2");
...
console.log(sessionStorage.getItem("mostraQTYS") )
let resultLimit = ref(sessionStorage.getItem("mostraQTYS") ?? 50);
const schedeQTY = ref();
...
watch(
  ...
  () => {
    getUserData();
    collection.value = "serie_italiana";
...
watch(selectedInterface, () => {
  sessionStorage.setItem("currentViewS", selectedInterface.value);
  //IMAGES
});
onMounted(async () => {
  if (piano.value !== "all") {
  ...
  currentQueryCache.value = JSON.parse(sessionStorage.getItem("mySearchSerie"));
  currentPageCache.value = sessionStorage.getItem("currentPageSerie");
  selectedInterface.value = sessionStorage.getItem("currentViewS");
  advancedSearchInput.value = sessionStorage.getItem("filterTableS");
...
async function fetchRelationalItems() {
  ...
  // TOTAL RESULTS
  schedeQTY.value = await directus.items("serie_italiana").readByQuery({
    fields: ["COUNT(*)"],
    limit: -1,
  });
}
function infoQty(qty) {
  if (qty !== undefined) {
    resultLimit.value = Number(qty);
    sessionStorage.setItem("mostraQTYS", resultLimit.value);
  }
...
function skipPage(page) {
  ...
  sessionStorage.setItem("currentPageSerie", currentPage.value);
...
async function fetchData(settings) {
  ...
  sessionStorage.removeItem("filterTableS");
  ...
  let query = {
    ...
    filter: {},
    sort: "foglio",
    fields: "id,nome,foglio,quadrante,orientamento,scala,anno,link",
  };
  if (piano.value !== "all") {
  ...
  try {
    ...
    if (form.value.foglio !== "") {
      query["filter"]["foglio"] = { _istarts_with: form.value.foglio };
    }
    ...
    if (form.value.scala !== "") {
      query["filter"]["scala"] = { _contains: form.value.scala };
    }

    if (form.value.anno !== "") {
      query["filter"]["anno"] = { _contains: form.value.anno };
    }
    ...
    sessionStorage.setItem("mySearchSerie", JSON.stringify(query));
    console.log(query);
...
async function fetchIcons() {
  ...
  const responseSaved = await directus.items("pref_s").readByQuery(query2);

  for (let x = 0; x < responseSaved.data.length; x++) {
    try {
      let iconSaved = document.getElementById(
        "saveButton-" + responseSaved.data[x].id_serie
      );
  ...
  try {
    const lastModifiedCarte = sessionStorage.getItem("lastModifiedSerie");
...
function clearData() {
  ...
  sessionStorage.removeItem("mySearchSerie");
  sessionStorage.removeItem("currentPageSerie");
...
async function deleteItem(item) {
  await directus
    .items("serie_italiana")
    .updateOne(item.id, { status: "bozza" });

  fetchData();
}
function onEditClicked(item) {
  sessionStorage.setItem("lastModifiedSerie", item.id);
...
async function onSaveClicked(item) {
  ...
  if (iconSaved.classList.contains("bi-heart")) {
    iconSaved.className = "bi bi-heart-fill";
    await directus.items("pref_s").createOne({
      id_serie: item.id,
      nome: item.nome,
      foglio: item.foglio,
    ...
    iconSaved.className = "bi bi-heart";
    let response = await directus.items("pref_s").readByQuery(query);
    await directus.items("pref_s").deleteOne(response.data[0].id);
  }
}
...
function filterTable() {
  console.log(advancedSearchInput.value);
  if (advancedSearchInput.value !== null && advancedSearchInput.value !== "") {
    sessionStorage.setItem("filterTableS", advancedSearchInput.value);
  }
