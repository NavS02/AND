<template v-if="field.invisible !== 'true'" >
    <slot name="label">
        <label :for="`field-${field.name}`" class="form-label" v-html="field.label"></label>
    </slot>
    <template v-if="item && field.invisible !=='true'">
    ...
    <template v-else>
        <div class="buttons d-flex gap-2 border border-muted rounded p-2" v-if="field.edit !== 'false' && field.invisible !== 'true'">
            ...
            <button class="btn btn-sm btn-primary" @click="onSelectExistingClicked"  v-if="field.invisible !== 'true'" >
...
const useQuery = (query) => {
  ...
  if (related == "directus_roles") {
    ...
    params.filter.description = {
      _eq: "cartografico", // only show fotografico roles
    };
    return params;
  } else {
    const params = {};
