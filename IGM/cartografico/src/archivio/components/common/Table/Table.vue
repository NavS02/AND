...
export default {
    ...
    computed: {
    _items() {
        const items = (this.items && Array.isArray(this.items)) ? [...this.items] : [];
        
        if (this.sorts.length === 0) return items;
    ...
    mounted() {
        const savedSorts = sessionStorage.getItem('sorts')
        if (savedSorts) {
            this.sorts = JSON.parse(savedSorts)
        }
    },
    methods: {
        sortIndex(field) {
        ...
        if (index < 0) {
            // Si no hay un sort previo, añadir uno
        ...
        sessionStorage.setItem('sorts', JSON.stringify(this.sorts))
    },
    }
}
</script>
