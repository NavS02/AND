<template>
  <main id="main" class="main">
    <div v-if="loaded">
      ...
      <h4
        class="text-center"
        v-if="collection !== 'directus_users' && collection !== 'inventario'"
      >
        Scheda n. {{ id }}
      </h4>
      <div v-if="collection == 'inventario'">
        <h4 class="text-center">
          {{ codesData }}
        </h4>
        <h6 class="text-center">Scheda n. {{ id }}</h6>
      </div>
        ...
        <template v-slot:footer="{ data }">
          <div class="buttons">
            ...
            <button
              ...
              v-if="collection === 'inventario' && item.oa"
            >
              <font-awesome-icon :icon="['fa-solid', 'fa-paintbrush']" />
              <span class="ms-1">Edita scheda OA</span>
            </button>
          </div>
        </template>
      </Form>
...
watch(
  ...
  async () => {
    ...
    setTimeout(async () => {
      ...
      loaded.value = true;
      fecthImage();
      codesData.value = formattedItem();

      if (collection.value == "inventario") {
        let oa = fields.value.find((field) => field.name === "oa");
        if (oa) {
          watch(
            () => oa.value,
            (newValue, oldValue) => {
...
function ckeckOA(newValue) {
  console.log(`OA changed : ${newValue}`);
}
function formattedItem() {
  let parts = [];

  if (item.value.inventario_1973 && item.value.inventario_1973 !== "") {
    parts.push(item.value.inventario_1973);
  }
  if (item.value.IBS && item.value.IBS !== "") {
    parts.push(item.value.IBS);
  }
  if (item.value.invn && item.value.invn !== "") {
    parts.push(item.value.invn);
  }
...
async function onEditOpera() {
  router
    .push({
      name: "editItemOA",
      params: { id: item.value.oa.id, collection: "opera" },
    })
    .then(() => {
      location.reload();
    });
...
async function updateAPP() {
  ...
  let itemSec = Myitem.data[0];

  const opereMtc = await directus.items("inventario_inv_materia").readByQuery({
    filter: {
      inventario_id: { _in: itemSec.id },
    },
    limit: -1,
  });
  const idOpereMTC = opereMtc.data.map(({ inv_materia_id }) => inv_materia_id);
  ...
  const inv_collocazione = await directus
    .items("inv_collocazione")
    .readByQuery({
      filter: {
        id: { _in: item.value.inv_collocazione },
      },
      limit: -1,
    });
  let mtcAPP = "";
  ...
  const response2 = await directus.items("app").updateOne(item.app, {
    icona: itemSec.icona,
    invn: itemSec.invn,
    autn: item.value.autore,
    inv_oggetto: item.value.inv_oggetto,
    piano: itemSec.inv_piano,
    materia: mtcAPP,
    sala: itemSec.inv_sala,
    parete: itemSec.parete,
    sgti: item.value.soggetto,
    descrizione: itemSec.descrizione,
    specifiche: itemSec.ubi_specifica,
    id_opera: itemSec.id,
  });
}
async function updateWeb() {
  ...
  try {
    ...
    mtcAPP = "";
    console.log("Error fetching materia (WEB)" + error);
  }

  try {
    const oggettoValueApp = await directus.items("inv_oggetto").readByQuery({
      filter: {
        id: { _eq: itemSec.inv_oggetto },
      },
      limit: -1,
    });
    itemSec.inv_oggetto = oggettoValueApp.data[0].inv_oggetto;
  } catch (error) {
    itemSec.inv_oggetto = "";
    console.log("Error fetching ogtd (WEB)");
  }

  const response2 = await directus.items("web").updateOne(itemSec.web, {
    icona: itemSec.icona,
    autn: itemSec.autore,
    ogtd: itemSec.inv_oggetto,
    piano: itemSec.inv_piano,
    materia: mtcAPP,
    sala: itemSec.inv_sala,
    parete: itemSec.parete,
    sgti: itemSec.soggetto,
...
async function updateOA() {
  ...
  try {
    const idOpereStima = itemSec.inv_stima.map(
    ...
    itemSec.inv_stima = stimaValue.data;
    ...
    itemSec.inv_stima=stimaTempString
  } catch (error) {
    stimaOA = "";
  ...
  try {
    const idOpereMTC = itemSec.inv_materia.map(
      ({ inv_materia_id }) => inv_materia_id
    );
    console.log(idOpereMTC);
    const mtcValueApp = await directus.items("inv_materia").readByQuery({
      filter: {
        id: { _in: idOpereMTC },
      },
    ...
    mtcAPP = "";
    console.log("Error fetching materia (OA)" + error);
  }
...
item.inv_collocazione.inv_collocazione = ""; 
  ...
  console.log(itemSec);
  const response2 = await directus.items("opera").updateOne(itemSec.oa.id, {
    icona: itemSec.icona,
    inventario: itemSec.id,
    nctn: itemSec.nctn,
    ogtd_oa: itemSec.inv_oggetto.inv_oggetto,
    qntn: itemSec.quantita,
    sgti: itemSec.soggetto,
    deposito: itemSec.deposito,
    piano: itemSec.inv_piano,
    sala: itemSec.inv_sala,
    parete: itemSec.parete,
    specifiche: itemSec.ubi_specifica,
    acquisizione: itemSec.acquisizione,
    materia: mtcAPP,
    misure: itemSec.misure,
    autore_inv: itemSec.autore,
    data: itemSec.data,
    cdgs: itemSec.proprieta,
    dismesso: itemSec.dismesso,
    provenienza: itemSec.provenienza,
    prestito_inv: itemSec.prestito,
    invn: itemSec.invn,
    ibs: itemSec.IBS,
    inventario_1930: itemSec.inventario_1930,
    inventario_1973: itemSec.inventario_1973,
    collocazione_inv: itemSec.inv_collocazione.inv_collocazione,
    stima_inv: itemSec.inv_stima,
  });
}
