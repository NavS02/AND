<template>
  ...
  <main id="main">
    <div class="row d-flex">
      ...
      <br /><br />
      <div style="width: 900px; margin-top: 3%">
        <canvas id="ubicazioneGraph"></canvas>
      </div>
      <div style="width: 550px; margin-top: 3%; margin-left: 5%">
        <canvas id="salaGraph"></canvas>
      </div>
...
async function fetchDashboard() {
  ...
  dataCol.value.forEach((inventario) => {
    collocazioneData.value.forEach((collocazione) => {
      if (
        inventario.inv_collocazione == collocazione.id &&
        inventario.inv_collocazione !== null
      ) {
...
function handleBarClick(label) {
  ...
  const filteredDataCol = dataCol.value.filter(
    (item) => item.ubicazioneLabel == label
  );
...
async function loadChartSala(data) {
  ...
  salaChart = new Chart(ctx, {
    ...
    options: {
      ...
      onClick: (e) => {
        const activePoints = salaChart.getElementsAtEventForMode(
          e,
          "nearest",
          { intersect: true },
          false
...
function redirectToCerca(sala) {
  let filteredData = collocazioneData.value.filter(
    (item) =>
      item.inv_ubicazione == currentUbicazione.value && item.inv_sala == sala
  );
  if (filteredData[0] == undefined) {
    filteredData = collocazioneData.value.filter(
      (item) =>
        item.inv_ubicazione == currentUbicazione.value && item.inv_sala == null
    );
  }
  ...
  let query = {
    ...
    field: "*.*",
    filter: {
      inv_collocazione: { _eq: filteredData[0].id },
    },
  };
  router.push({ name: "searchArc", params: { piano: JSON.stringify(query) } });
}
</script>
