      ...
      <Form :fields="fields">
        <template v-slot:footer="{ data, fields }">
          <div class="buttons">
            ...
            <button
              class="btn btn-sm btn-warning"
              @click="onEditInventario(data)"
              v-if="collection === 'opera'"
            >
              <font-awesome-icon :icon="['fa-solid', 'fa-paintbrush']" />
              <span class="ms-1">Edita scheda inventario</span>
            </button>
          </div>
        </template>
      </Form>
...
async function onEditInventario(data) {
  router
    .push({
      name: "editItemArc",
      params: { id: item.value.inventario.id, collection: "inventario" },
    })
    .then(() => {
      location.reload();
    });
...
function goToList() {
  router.push({ name: "searchOA", params: { piano: "all" } });
}
