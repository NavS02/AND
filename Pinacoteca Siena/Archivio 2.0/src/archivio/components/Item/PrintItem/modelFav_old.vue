...
<td>Cronologia</td>
<td>
  <span v-if="opera?.cronologia?.length">
    <span v-for="(item, index) in opera.cronologia" :key="item.cronologia_id.id">
      {{ formatCronologia(item.cronologia_id) }}
      <span v-if="index < opera.cronologia.length - 1">; </span>
    </span>
  </span>
  <span v-else>-</span>
</td>
