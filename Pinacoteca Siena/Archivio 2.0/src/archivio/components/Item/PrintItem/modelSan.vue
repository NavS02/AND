...
export default {
  setup(props, context) {
    ...
    function changeItem() {
      ...
      router.push({ name: 'modelSanOA', params: { id: currentId }  })
      url.value=null;
        ...
        if (collection.value == "opera") {
          ...
          if (item.ogtd !== null) {
            try {
              ...
              const item_oggetto = await directus.items("ogtd_oa").readByQuery({
                limit: -1,
                filter: {
                  id: {
                    _in: idsO,
  ...
  try {
    ...
    response.value.inv = "";
          ...
          // STIMA
          if (item.stima_opera !== null) {
            try {
              const idsStim = item.data.map(({ stima_opera }) => stima_opera);
          ...
          if (item.cronologia !== null) {
            try {
              ...
              let totalCrono = "";
                ...
                const item = cronologia.data[index];

                const valores = [item.dtzg, item.dtsi, item.dtsf].filter(v => v != null && v !== "");

                if (valores.length > 0) {
                  totalCrono +=
                    " (" + (index + 1) + ") " + valores.join(", ");
                }
              }
