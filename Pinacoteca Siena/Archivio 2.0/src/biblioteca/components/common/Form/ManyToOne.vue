<template>
    ...
    <template v-if="item">
        <div class="item d-flex p-2">
            ...
            <div class="d-flex ms-auto gap-2" v-if="field.edit !=='false'">
                ...
                <button class="btn btn-sm btn-light text-danger ms-auto" @click="onRemoveClicked" v-if="field.name !== 'bib_utente'">
    ...
    <Drawer ref="addExistingRef">
        ...
        <SelectExisting :collection="related" :useQuery="useQuery" :sort=sort v-slot="{items}">
...
const {
    ...
    filter,
    sort // filter function used when searching for an item by text
} = field.value
