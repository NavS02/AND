  ...
  <main id="main" class="main" v-if="userRolePermissions">
    ...
    <div class="col-12">
      <div class="row">
        ...
        <div class="col-md-6">
          <label class="form-label" :for="`form-ufficio`">Ufficio:</label>
          <input
            type="text"
            class="form-control"
            v-model="form.ufficio"
            :id="`form-ufficio`"
            aria-label="Small"
            aria-describedby="inputGroup-sizing-sm"
          />
      ...
      <div class="row">
        ...
        <div class="col-md-6">
          <label class="form-label" :for="`form-telefono`">Telefono:</label>
          <input
            type="text"
            class="form-control"
...
const storageManager = useStorage("advanced-search-form-USER");
const mtct_F = ref([]);
...
watch(
  ...
  () => {
    getUserData();
    collection.value = "bib_utente";
       form.value = JSON.parse(sessionStorage.getItem('advanced-search-form-USER')) ?? makeEmptyForm();
...
async function fetchData(settings) {
  ...
  let query = {
    limit: -1,
    filter: {},
    fields: "id,bib_nome,bib_cognome,bib_ufficio,bib_email,bib_telefono",
  };
  if (piano.value !== "all") {
  ...
  try {
    ...
    if (form.value.ufficio !== "") {
      query["filter"]["bib_ufficio"] = { _contains: form.value.ufficio };
    }
    if (form.value.email !== "") {
      query["filter"]["bib_email"] = { _eq: form.value.email };
    }
    if (form.value.telefono !== "") {
      query["filter"]["bib_telefono"] = { _eq: form.value.telefono };
    }

    // INVENTARIO
    if (form.value.nome !== "") {
      query["filter"]["_or"] = [
        { bib_nome: { _contains: form.value.nome } },
        { bib_cognome: { _contains: form.value.nome } },
      ];
    }

    query["filter"]["status"] = { _eq: "attivo" };
    const response = await directus.items('bib_utente').readByQuery(query);
...
async function fetchRelations() {
  ...
  //               inventarioNames += relationalInv.nome + " ";
  //             }
  //           });
  //         }
  //       });
