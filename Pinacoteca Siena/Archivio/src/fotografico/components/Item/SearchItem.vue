<template>
  <main id="main" class="main">
    ...
    <div class="col-12">
      ...
      <div class="row">
        ...
        <div class="col-md-6">
          <label class="form-label" :for="`form-sgtt`"
            >Soggetto/Titolo (SGTI/SGLT):</label
          >
          <input
            type="text"
            class="form-control"
      ...
      <div class="row">
        <div class="col-md-6">
          <label class="form-label" :for="`form-mtct_F`"
            >Materia e tecnica (MTC):</label
          >
          <AutoComplete
            v-model="form.mtct_F"
            forceSelection
...
async function fetchData(settings) {
  ...
  let query = {
    limit: -1,
    filter: {},
    fields: "id,oa_sgti,oa_sgtt,oa_dtsf,oa_dtsi,inventario,opera_MNS,oa_autore",
  };
  if (piano.value !== "all") {
          ...
          if (relationalItem.id == itemautore) {
            autoreData.data.forEach((relationalfotografo) => {
              if (relationalItem.oa_autore_id == relationalfotografo.id) {
                console.log(relationalfotografo.autn);
