  ...
  <div>
    ...
    <div class="container">
      <button
        ...
        style="width: 100%; border-color: #5a00cd"
      >
        <span style="color: #5a00cd">IN A SOUNDSCAPE OF COLOURS</span>
        &nbsp;<font-awesome-icon icon="right-long" color="#5A00CD" />
      </button>
    </div>
    <br />
    <div class="container">
      <div v-for="(item, index) in items" :key="index">
        <div>
          <div
            ...
            :style="{ backgroundColor: !item.opera ? 'white' : 'white' }"
            ...
            <div class="card-body" style="margin: -15px !important">
              <div class="row">
                ...
                <div class="text-right col-6">
                  ...
                  <span v-else-if="lingua == 'en'" style="font-size: 16px">
                    {{ item.titolo_eng || item.titolo }}
                  </span>
                  <span v-else-if="lingua == 'fr'" style="font-size: 16px">
                    {{ item.titolo_fra || item.titolo }}
                  </span>
                  <span v-else-if="lingua == 'es'" style="font-size: 16px">
                    {{ item.titolo_spa || item.titolo }}
                  </span>
                ...
                <div style="text-align: left">
                  <hr style="margin: 2px" />
                  <div style="line-height: 1.3" v-if="lingua == 'it'">
                    {{ item.descrizione }}
                  </div>
                </div>
              </div>
...
async function fetchItems() {
  if (tipologia.value == "kid") {
    ...
    let filteredItems = data.value.filter(
      (item) => item.tipo_visita && item.tipo_visita.includes("In a Soundscape")
    );
    items.value = filteredItems;
  }
