<template>
  <main id="main" class="main">
    <div v-if="loaded">
      ...
      <div v-if="collection === 'opera'">
        <h2 class="text-center">{{ invn }}</h2>
      </div>
      <div v-else>
...
// COLLOCAZIONE
  ...
  let appCollocazioneSala;
    ...
    try {
      const collocazioneInMuseo = await directus.items("collocazioneInMuseo").readByQuery({
        filter: {
...
async function updateAPP() {
  ...
  try {
    ...
    try {
      ...
      console.log(collocazioneInMuseo.data[0].sala)
      if(collocazioneInMuseo.data[0].sala !== null){
